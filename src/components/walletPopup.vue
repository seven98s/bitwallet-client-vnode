<template>
	<view class="walletPopup" v-if="show">
...
onMounted(async () => {
	...
	const accountChains = WalletData.accountChainMap[param.chainActive];

	for (const accountChain of accountChains) {
		const { name, address, type } = accountChain;
		
		try {
			const rs = await BitAccount.getAssets(address, [], true, 1, 100);
			const datas = rs.data;
	...
	// 是否存在钱包助记词
	param.existsMnemonic = WalletData.existsWalletMnemonic();
...
const emit = defineEmits<{
  (e: 'walletPopupshow', value: boolean): void;
}>();
