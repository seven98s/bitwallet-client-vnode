...
const loadTransfers = async () => {
	...
	try {
		const rs = await BitAccount.getTransfers(account.address, "", 1, 100) as any;
		const datas = rs.data;
		datas.map((data: any) => {
			accounts.value.push({
				chain: 'BIT',
...
const emit = defineEmits(['transferPopupshow', 'chooseAccountValue'])

const close = () => {
	emit('transferPopupshow', false)
}
const chooseAccount = (address: string) => {
	emit('transferPopupshow', false)
	emit('chooseAccountValue', address)
}

const handleNavClick = async (chainActive: string) => {
