...
const init = async () => {
	...
	show.totalPersonPledgeNum = BitInstance.bitweb.fromBit(Number(totalPersonPledgeNum)).toString();

	param.recordsEmpty = records.value.length === 0;
...
const unlock = async (expireTime: number) => {
	UniUtil.loadShow();
	...
	try {
		const rs = await BitInstance.bitweb.trx.unfreezeBalance('BANDWIDTH', {}, WalletData.account.address);
		if (rs.result) {
