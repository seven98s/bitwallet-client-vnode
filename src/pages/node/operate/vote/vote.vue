...
onLoad((query: any) => {
	...
	if (query.address != undefined) {
		param.srAddress = query.address;
	}
});

onShow(async () => {
	const chainAccount = await BitInstance.bitweb.trx.getUnconfirmedAccount(WalletData.account.address);
	show.bpNum = chainAccount.frozen !== undefined && chainAccount.frozen.length > 0 ? BitInstance.bitweb.fromBit(parseInt(chainAccount.frozen[0].frozen_balance)).toString() : '0';
	console.log(chainAccount)
})

const confirm = async () => {
	UniUtil.loadShow();
	const { srAddress, bpNum } = param;
	// check
	...
	if (BigInt(bpNum) > BigInt(show.bpNum)) {
	...
	try {
		const rs = await BitInstance.bitweb.transactionBuilder.vote(votes);
		const sign = await BitInstance.bitweb.trx.sign(rs);

		try {
			const voteRs = await BitInstance.bitweb.trx.sendRawTransaction(sign);
			console.log(voteRs)
			if (voteRs.result) {
