...
const manageWalletList = reactive([
	{
		id: Date.now(),
		address: ''
	}
...
const createMultiSignWallet = async () => {
	UniUtil.loadShow();
	const { address, threshold } = param.multiSignWallet;

	//check
	const hasEmptyAddress = manageWalletList.some((item, index) => {
	...
	const addressArr = manageWalletList.map(item => item.address);
	...
	addressArr.map((address) => {
		...
		activeAddress.value.push({ address: BitInstance.bitweb.address.toHex(address), weight: 1 });
	});
	if (addressFlag) {
	...
	let ownerPermission: Permission = { type: 0, permission_name: 'owner', threshold: parseInt(threshold), keys: activeAddress.value };

	let activePermission: Permission = { type: 2, permission_name: 'active0', threshold: parseInt(threshold), keys: activeAddress.value };
	activePermission.operations = '7fff1fc0037e0000000000000000000000000000000000000000000000000000';
	try {
		const rs = await BitInstance.bitweb.transactionBuilder.updateAccountPermissions(
			owner,
			ownerPermission,
			undefined,
			[activePermission]
		...
		//发送原始交易
		try {
			const multiSignRs = await BitInstance.bitweb.trx.sendRawTransaction(sign);
...
const addManageWallet = () => {
	let tempObj = {
		id: Date.now(),
		address: ''
	}
	manageWalletList.push(tempObj)
}

const deleteManageWallet = (id: number)  => {
	const index = manageWalletList.findIndex(item => item.id === id)
	if (index) {
		manageWalletList.splice(index, 1)
	} else {
