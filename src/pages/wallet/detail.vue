...
<script lang="ts">
	...
	export default Vue.extend({
		...
		onShow() {
			...
			this.show.address = ShowUtil.omitAddress(address, 8, 8);
			this.show.name = name;
			this.value.address = address;

			// 是否展示按钮
			if (secretInfo.privateKey !== undefined) {
				this.param.isShowPriKey = secretInfo.privateKey != '';
			}
			if (secretInfo.mnemonic !== undefined) {
				this.param.isShowMnemonic = secretInfo.mnemonic != '';
			}
		},
		methods: {
			copyAddress() {
				UniUtil.copy(this.value.address);
			},
			async exportFun(type : number) {
				const rs = await new Promise<string>(async (resolve, reject) => {
					UniUtil.loadHide();
						...
						confirm: async (data : string) => {
							...
							if (type === 1) {
								await WalletData.privateKey(data).then((privateKey) => {
									resolve(privateKey); // .substring(2, privateKey.length)
								}).catch((e : any) => {
									// reject(e);
								});
							} else if (type === 2) {
								await WalletData.mnemonic(data).then((mnemonic) => {
									resolve(mnemonic);
								}).catch((e : any) => {
									// reject(e);
								});
				...
				Vue.prototype.$store.commit('changePasswordPopConfig', { show: false });
			...
			async deleteWallet() {
				if (WalletData.account.type === 2) {
					...
					const rs = await new Promise<void>(async (resolve, reject) => {
						UniUtil.loadHide();
						Vue.prototype.$PasswordPopup({
							title: '验证密码',
							show: true,
							...
							confirm: async (data : string) => {
								...
								await WalletData.privateKey(data).then(async () => {
									await WalletData.removeAccount();
									uni.reLaunch({
										url: '../home/index'
									});
